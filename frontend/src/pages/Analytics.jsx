...
const Analytics = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      const [summaryRes, revenueRes, insightsRes, contentRes] = await Promise.all([
        axios.get(`${API}/analytics/summary?period=${selectedPeriod}`),
        axios.get(`${API}/analytics/revenue?period=${selectedPeriod}`),
        axios.get(`${API}/analytics/insights`),
        axios.get(`${API}/analytics/content`)
      ]);
          ...
          <div className="p-6 rounded-2xl surface-container-lowest col-span-1">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-[hsl(var(--success))] text-sm font-medium">
                ...
                <span>+{summary?.revenue_change || 0}%</span>
              </div>
            </div>
