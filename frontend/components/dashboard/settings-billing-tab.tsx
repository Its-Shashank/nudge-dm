import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Icon } from "@/components/ui/icon";
import { PLAN_CATALOG } from "@/lib/constants/billing";
import type { SubscriptionDetails } from "@/lib/api/billing";

export interface SettingsBillingTabProps {
  subscription: SubscriptionDetails;
}

export function SettingsBillingTab({ subscription }: SettingsBillingTabProps) {
  const plan = PLAN_CATALOG[subscription.plan] ?? PLAN_CATALOG.FREE;

  return (
    <div className="rounded-2xl border border-line bg-white p-6 shadow-xl">
      <h2 className="mb-6 text-headline-md text-ink">Subscription Summary</h2>

      <div className="flex flex-col gap-5 rounded-xl border border-line p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.1em] text-violet">Current Plan</p>
          <p className="mt-1 text-headline-lg text-ink">{plan.name}</p>
          <p className="mt-1 text-body-md capitalize text-on-surface-variant">
            Status: {subscription.status}
          </p>
        </div>

        <Link href="/dashboard/billing">
          <Button type="button" variant="secondary" className="w-full sm:w-auto">
            Go to Billing Portal
            <Icon name="arrow_forward" className="text-[16px]" />
          </Button>
        </Link>
      </div>
    </div>
  );
}
